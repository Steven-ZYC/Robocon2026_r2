#!/usr/bin/env bash
# ============================================================================
# red_fsm.sh — Red field full FSM (5 weapon head rack points)
#
# 用法: 在终端中直接输入 red_fsm.sh 或通过别名 r
# ============================================================================
...
MISSION_FILE="$WS/src/navigation/routes/red/full_fsm.yaml"
...
echo "[red_fsm] Red field full FSM: point 1 → point 5"
echo "[red_fsm] chassis: $CHASSIS_CAN_DEVICE"
echo "[red_fsm] sensor:  $SENSOR_ARDUINO_PORT"
echo "[red_fsm] arm:     $ARM_ARDUINO_PORT"
echo "[red_fsm] mission: $MISSION_FILE"
echo ""

if [ ! -e "$CHASSIS_CAN_DEVICE" ]; then
  echo "[red_fsm] WARNING: $CHASSIS_CAN_DEVICE 不存在"
fi
if [ ! -e "$SENSOR_ARDUINO_PORT" ]; then
  echo "[red_fsm] WARNING: $SENSOR_ARDUINO_PORT 不存在"
fi
if [ ! -e "$ARM_ARDUINO_PORT" ]; then
  echo "[red_fsm] WARNING: $ARM_ARDUINO_PORT 不存在"
fi
echo ""
...
echo "[red_fsm] 5 窗口已启动 (无 plot_debug)"
