...
stages:
  ...
  - id: move_to_rack
    ...
    profile: red_area
    timeout_s: 3.5
    # Early quit: middle -> point 1 过程中，如果底盘 motor 1 torque 突增，视为已接触 rack/到达 point 1。
    # Timeout: 3.5 秒仍未到点/未触发 torque 时，自动进入 point 1 pickup 流程。
    torque_arrival:
  ...
  # ================================================================
  # ---- 失败安全姿态：M5=right, M6=up, gripper=open, lift=low ----
