/*
  ...
  Host -> Arduino:
    [0,0,0]\n  // gripper open,  lift low,  stopper low
    [1,0,0]\n  // gripper close, lift low,  stopper low
    [0,1,0]\n  // gripper open,  lift high, stopper low
    [0,0,1]\n  // gripper open,  lift low,  stopper high
    [1,0,1]\n  // gripper close, lift low,  stopper high
    STATUS\n
    ALL_OFF\n
  ...
  Pin / relay / valve mapping:
    Pneu 1 / Relay 1 / Gripper -> D5
        logical 0 = HIGH  // open
        logical 1 = LOW   // close

      Pneu 2 / Relay 2 / Lift -> D6
        logical 0 = LOW   // low
        logical 1 = HIGH  // high

      Pneu 3 / Relay 3 / Stopper -> D8
        logical 0 = LOW   // low
        logical 1 = HIGH  // high
...
//   1 = high
//
// This table defines the Arduino output level for logical command 1.
const byte pneuLevelWhenLogicalOne[NUM_PNEU] = {
  LOW,   // Pneu 1 / Gripper: 1 = close
  HIGH,  // Pneu 2 / Lift:    1 = high
  HIGH   // Pneu 3 / Stopper: 1 = high
};
...
void setup() {
  ...
  for (byte i = 0; i < NUM_PNEU; i++) {
    ...
    digitalWrite(pneuPins[i], pneuLogicalZeroLevel(i));
  }
...
void applyPneuStates(bool newStates[]) {
  for (byte i = 0; i < NUM_PNEU; i++) {
    ...
    if (pneuStates[i]) {
      digitalWrite(pneuPins[i], pneuLogicalOneLevel(i));
    } else {
      digitalWrite(pneuPins[i], pneuLogicalZeroLevel(i));
    }
  }
...
void allPneuOff() {
  // Safe default mechanical state:
  // [0,0,0] = gripper open, lift low, stopper low
  for (byte i = 0; i < NUM_PNEU; i++) {
    pneuStates[i] = false;
    digitalWrite(pneuPins[i], pneuLogicalZeroLevel(i));
  }
...
byte pneuLogicalOneLevel(byte index) {
  return pneuLevelWhenLogicalOne[index];
}

byte pneuLogicalZeroLevel(byte index) {
  return pneuLevelWhenLogicalOne[index] == HIGH ? LOW : HIGH;
}
