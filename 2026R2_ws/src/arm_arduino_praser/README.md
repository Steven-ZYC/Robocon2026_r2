...
| Topic | 类型 | 格式 | 说明 |
|---|---|---|---|
| `arm/pneu_command` | `Float32MultiArray` | `[arm_stopper, arm_lift, arm_gripper]` | 气动阀指令，值域 0.0/1.0 |

### 发布 Topic

| Topic | 类型 | 格式 | 说明 |
|---|---|---|---|
| `arm/pneu_ack` | `Float32MultiArray` | `[arm_stopper, arm_lift, arm_gripper]` | Arduino 回传的当前气动阀实际状态 |
...
| 参数 | 类型 | 默认值 | 单位 | 说明 |
|---|---|---|---|---|
| `port` | string | `/dev/ttyUSB0` | - | Arduino 串口设备路径（建议使用 `/dev/arm_arduino` udev symlink） |
...
| `default_pneu` | float array | `[0.0, 0.0, 0.0]` | - | 启动默认气动状态（全关） |
...
指令值：`0.0` = 关闭 / `1.0` = 开启，非 0/1 值会以 `>= 0.5` 规则转换为 0 或 1 并输出 WARN。
...
# 手动发送气动指令
ros2 topic pub --once arm/pneu_command std_msgs/msg/Float32MultiArray "{data: [0.0, 0.0, 1.0]}"

# 全部关闭
ros2 topic pub --once arm/pneu_command std_msgs/msg/Float32MultiArray "{data: [0.0, 0.0, 0.0]}"
```
